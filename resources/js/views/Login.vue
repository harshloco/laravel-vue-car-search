<template>
  <div id="loginForm" class="pt-5">
    <b-form
      v-on:submit.prevent="signIn(form.identity,form.password)"
      class="mx-auto justify-content-center w-50"
    >
      <b-form-group id="loginErrors" class="errorsBlock" v-show="containsErrors()">
        <ul v-for="error in getErrors()" :key="error">
          <li>{{ error.message }}</li>
        </ul>
      </b-form-group>
      <b-form-group id="loginGroup" horizontal label="Login: " label-for="login">
        <b-form-input
          id="login"
          name="login"
          type="text"
      ...
      <b-form-group id="passwordGroup" horizontal label="Password:" label-for="password">
        <b-form-input
          id="password"
          name="password"
          type="password"
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          this.$store
            ...
            .then(() => this.$router.push({ name: "cars" }));
        }
      });
    }
  }
